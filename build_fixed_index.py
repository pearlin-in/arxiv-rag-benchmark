"""
Phase 3 (fixed-size chunks): embeds every chunk in data/chunks_fixed.jsonl
using BAAI/bge-base-en-v1.5 and stores the embeddings in a persistent
ChromaDB collection ("fixed_chunks") using cosine similarity.

Usage:
    pip install sentence-transformers chromadb
    python build_fixed_index.py
"""
...
CHUNKS_PATH = Path("data/chunks_fixed.jsonl")
...
def clean_metadata(chunk: dict) -> dict:
    ...
    metadata = {
        ...
        "token_start": chunk.get("start_token", chunk.get("token_start")),
        "token_end": chunk.get("end_token", chunk.get("token_end")),
    }
    ...
    for key, value in metadata.items():
        if value is None:
            ...
            cleaned[key] = "" if key == "paper_id" else -1
        else:
            cleaned[key] = value
    return cleaned
